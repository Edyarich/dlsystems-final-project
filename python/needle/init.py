...
def xavier_uniform(fan_in, fan_out, shape=None, gain=1.0, **kwargs):
    ...
    border = gain * (6 / (fan_in + fan_out))**0.5
    return rand(*shape, low=-1.0, **kwargs) * border
...
def kaiming_uniform(fan_in, fan_out, shape=None, nonlinearity="relu", gain=2**0.5, **kwargs):
    ...
    return rand(*shape, low=-1.0, **kwargs) * bound


def kaiming_normal(fan_in, fan_out, shape=None, nonlinearity="relu", gain=2**0.5, **kwargs):
    assert nonlinearity == "relu", "Only relu supported currently"
