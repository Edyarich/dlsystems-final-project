...
def compute_gradient_of_variables(output_tensor, out_grad):
    ...
    for node in reverse_topo_order:
        ...
        # if type(node.grad) == TensorTuple and len(node.grad) == 1:
        #     print('node.op =', node.op)
        #     print('node.grad =', node.grad)
        #     print('node =', node)
        #     print('neighb_grads =', neighb_grads)

        if len(node.inputs) == 1:
            neighb_grads = [neighb_grads]
            
        for neighb, neighb_to_node_grad in zip(node.inputs, neighb_grads):
            if neighb not in node_to_output_grads_list:
                node_to_output_grads_list[neighb] = []

            node_to_output_grads_list[neighb].append(neighb_to_node_grad)
...
def find_topo_sort(node_list: List[Value]) -> List[Value]:
    ...
    visited = dict(zip(node_list, numpy.full(len(node_list), NodeColor.WHITE)))
    ordered_nodes = list()

    for node in node_list:
        if visited[node] == NodeColor.WHITE:
            topo_sort_dfs(node, visited, ordered_nodes, NodeColor)

    return ordered_nodes


def topo_sort_dfs(node, visited, topo_order, NodeColor):
    """Post-order DFS"""
    visited[node] = NodeColor.GRAY

    for neighb in node.inputs:
        if neighb not in visited:
            visited[neighb] = NodeColor.WHITE

        if visited[neighb] == NodeColor.GRAY and id(neighb) != id(node):
            assert False, "Computational graph contains cycle!!!"
        
        if visited[neighb] == NodeColor.WHITE:
            topo_sort_dfs(neighb, visited, topo_order, NodeColor)
