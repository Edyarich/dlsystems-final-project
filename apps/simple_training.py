...
def epoch_general_cifar10(dataloader, model, loss_fn=nn.SoftmaxLoss(), opt=None):
    ...
    for batch in dataloader:
        ...
        preds = outp.numpy().argmax(axis=-1)
        batch_err = np.mean(preds == y_batch.numpy())
        avg_acc += batch_err
        avg_loss += loss.numpy()
...
def train_cifar10(model, dataloader, n_epochs=1, optimizer=ndl.optim.Adam,
    ...
    for ep_num in range(n_epochs):
        tr_acc, tr_loss = epoch_general_cifar10(dataloader, model, loss_fn, optimizer)

        if ep_num + 1 == n_epochs:
...
def evaluate_cifar10(model, dataloader, loss_fn=nn.SoftmaxLoss):
    ...
    test_acc, test_loss = epoch_general_cifar10(dataloader, model, loss_fn)
