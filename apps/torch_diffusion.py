...
class Unet(nn.Module):
    ...
    def forward(self, x: Tensor, timestep: Tensor) -> Tensor:
        ...
        for up in self.ups:
            ...
            x = torch.stack((x, residual_x), dim=2).reshape(
...
class SinusoidalPosEmb(nn.Module):
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        emb = torch.exp(Tensor(range(half_dim), device=device) * -emb)
        emb = x.broadcast_to((x.size, 1)) @ emb.broadcast_to((1, emb.size))
        emb = torch.stack(
            (torch.sin(emb), torch.cos(emb)), dim=2
...
class Diffusion:
    ...
    def p_sample(self, model, x, t, t_index):
        betas_t = extract(self.betas, t, x.shape).broadcast_to(x.shape)
        ...
        if t_index == 0:
            ...
            noise = torch.randn(*x.shape, device=x.device, requires_grad=False)
            # Algorithm 2 line 4:
...
def extract(x, t, x_shape):
    ...
    device = x.device
    out_handle = torch.zeros((batch_size,))
...
def linear_beta_schedule(timesteps, device=None):
    ...
    beta_end = scale * 0.02
    return Tensor(np.linspace(beta_start, beta_end, timesteps),
